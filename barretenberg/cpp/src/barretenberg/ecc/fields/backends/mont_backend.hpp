...
//
// Picks one of the 5 concrete backends based on platform and build flags.
...
//   3. WASM RNE SIMD   — WASM with MONTMUL_VARIANT_RNE defined (R = 2^256)
//   4. WASM FMA SIMD   — WASM with MONTMUL_VARIANT_FMA defined (R = 2^264)
//   5. WASM int29     — WASM fallback (9 × 29-bit schoolbook, R = 2^261)
...
#elif defined(MONTMUL_VARIANT_RNE) && defined(__wasm__)
    RneBackend<Params>;
